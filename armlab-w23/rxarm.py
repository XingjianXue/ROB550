...
#   @_ensure_initialized
    ...
    def get_ee_pose(self):
        ...
        """
        dh_table = np.array([
            [0,      np.pi/2,  103.91,  np.pi/ 2     ],
            [205.73, 0,           0,      1.325      ],
            [200,    0,           0,     -1.325      ],
            [0,      np.pi/2,     0,      np.pi/2    ],
            [0,      0,           174.15, 0          ]
        ])
        T = FK_dh(dh_table, [self.get_positions()[0], self.get_positions()[1], self.get_positions()[2], self.get_positions()[3], self.get_positions()[4]], 5)
       

        return get_pose_from_T(T).tolist()  
