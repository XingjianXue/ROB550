...
// need to be packed in order to make the byte copy from numpy and C to play nicely
typedef struct __attribute__((__packed__ )) mbot_wheel_ctrl{
    uint64_t utime;
...
int mbot_wheel_ctrl_t_deserialize(uint8_t* src, mbot_wheel_ctrl_t* dest);
int mbot_wheel_ctrl_t_serialize(mbot_wheel_ctrl_t* src, uint8_t* dest);
#endif
